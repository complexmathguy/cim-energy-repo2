...
public class LoadMotorAggregate {  
    ...
    public void handle(UpdateLoadMotorCommand command) throws Exception {
    	...
    	UpdateLoadMotorEvent event = new UpdateLoadMotorEvent(command.getLoadMotorId(), command.getD(), command.getH(), command.getLfac(), command.getLp(), command.getLpp(), command.getLs(), command.getPfrac(), command.getRa(), command.getTbkr(), command.getTpo(), command.getTppo(), command.getTv(), command.getVt(), command.getLoadAggregate());        
    ...
    @CommandHandler
    public void handle(AssignLoadAggregateToLoadMotorCommand command) throws Exception {
    	LOGGER.info( "Handling command AssignLoadAggregateToLoadMotorCommand" );
    	
    	if (  loadAggregate != null && loadAggregate.getLoadAggregateId() == command.getAssignment().getLoadAggregateId() )
    		throw new ProcessingException( "LoadAggregate already assigned with id " + command.getAssignment().getLoadAggregateId() );  
    		
        apply(new AssignLoadAggregateToLoadMotorEvent(command.getLoadMotorId(), command.getAssignment()));
    }

    @CommandHandler
    public void handle(UnAssignLoadAggregateFromLoadMotorCommand command) throws Exception {
    	LOGGER.info( "Handlign command UnAssignLoadAggregateFromLoadMotorCommand" );

    	if (  loadAggregate == null )
    		throw new ProcessingException( "LoadAggregate already has nothing assigned." );  

    	apply(new UnAssignLoadAggregateFromLoadMotorEvent(command.getLoadMotorId()));
    }
    ...
    void on(UpdateLoadMotorEvent event) {
        ...
        this.tv = event.getTv();
        this.vt = event.getVt();
        this.loadAggregate = event.getLoadAggregate();
    }   
	...
	// single associations
    @EventSourcingHandler
    void on(AssignLoadAggregateToLoadMotorEvent event ) {	
    	LOGGER.info( "Event sourcing AssignLoadAggregateToLoadMotorEvent" );
    	this.loadAggregate = event.getAssignment();
    }

	@EventSourcingHandler
	void on(UnAssignLoadAggregateFromLoadMotorEvent event ) {	
		LOGGER.info( "Event sourcing UnAssignLoadAggregateFromLoadMotorEvent" );
