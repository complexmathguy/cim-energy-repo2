...
public class ACDCConverterAggregate {  
    ...
    public void handle(UpdateACDCConverterCommand command) throws Exception {
    	...
    	UpdateACDCConverterEvent event = new UpdateACDCConverterEvent(command.getACDCConverterId(), command.getBaseS(), command.getIdleLoss(), command.getMaxUdc(), command.getMinUdc(), command.getNumberOfValves(), command.getRatedUdc(), command.getResistiveLoss(), command.getSwitchingLoss(), command.getValveU0(), command.getPccTerminal());        
    ...
    @CommandHandler
    public void handle(AssignPccTerminalToACDCConverterCommand command) throws Exception {
    	LOGGER.info( "Handling command AssignPccTerminalToACDCConverterCommand" );
    	
    	if (  pccTerminal != null && pccTerminal.getTerminalId() == command.getAssignment().getTerminalId() )
    		throw new ProcessingException( "PccTerminal already assigned with id " + command.getAssignment().getTerminalId() );  
    		
        apply(new AssignPccTerminalToACDCConverterEvent(command.getACDCConverterId(), command.getAssignment()));
    }

    @CommandHandler
    public void handle(UnAssignPccTerminalFromACDCConverterCommand command) throws Exception {
    	LOGGER.info( "Handlign command UnAssignPccTerminalFromACDCConverterCommand" );

    	if (  pccTerminal == null )
    		throw new ProcessingException( "PccTerminal already has nothing assigned." );  

    	apply(new UnAssignPccTerminalFromACDCConverterEvent(command.getACDCConverterId()));
    }
    ...
    void on(UpdateACDCConverterEvent event) {
        ...
        this.pccTerminal = event.getPccTerminal();
    }   
	...
	// single associations
    @EventSourcingHandler
    void on(AssignPccTerminalToACDCConverterEvent event ) {	
    	LOGGER.info( "Event sourcing AssignPccTerminalToACDCConverterEvent" );
    	this.pccTerminal = event.getAssignment();
    }

	@EventSourcingHandler
	void on(UnAssignPccTerminalFromACDCConverterEvent event ) {	
		LOGGER.info( "Event sourcing UnAssignPccTerminalFromACDCConverterEvent" );
    ...
    private Terminal pccTerminal = null;
    private ACDCConverterDCTerminal dCConductingEquipment = null;
