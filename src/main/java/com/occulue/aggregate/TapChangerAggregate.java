...
public class TapChangerAggregate {  
    ...
    public void handle(UpdateTapChangerCommand command) throws Exception {
    	...
    	UpdateTapChangerEvent event = new UpdateTapChangerEvent(command.getTapChangerId(), command.getHighStep(), command.getLowStep(), command.getLtcFlag(), command.getNeutralStep(), command.getNeutralU(), command.getNormalStep(), command.getTapChangerControl());        
    ...
    @CommandHandler
    public void handle(AssignTapChangerControlToTapChangerCommand command) throws Exception {
    	LOGGER.info( "Handling command AssignTapChangerControlToTapChangerCommand" );
    	
    	if (  tapChangerControl != null && tapChangerControl.getTapChangerControlId() == command.getAssignment().getTapChangerControlId() )
    		throw new ProcessingException( "TapChangerControl already assigned with id " + command.getAssignment().getTapChangerControlId() );  
    		
        apply(new AssignTapChangerControlToTapChangerEvent(command.getTapChangerId(), command.getAssignment()));
    }

    @CommandHandler
    public void handle(UnAssignTapChangerControlFromTapChangerCommand command) throws Exception {
    	LOGGER.info( "Handlign command UnAssignTapChangerControlFromTapChangerCommand" );

    	if (  tapChangerControl == null )
    		throw new ProcessingException( "TapChangerControl already has nothing assigned." );  

    	apply(new UnAssignTapChangerControlFromTapChangerEvent(command.getTapChangerId()));
    }
    ...
    void on(UpdateTapChangerEvent event) {
        ...
        this.tapChangerControl = event.getTapChangerControl();
    }   
	...
	// single associations
    @EventSourcingHandler
    void on(AssignTapChangerControlToTapChangerEvent event ) {	
    	LOGGER.info( "Event sourcing AssignTapChangerControlToTapChangerEvent" );
    	this.tapChangerControl = event.getAssignment();
    }

	@EventSourcingHandler
	void on(UnAssignTapChangerControlFromTapChangerEvent event ) {	
		LOGGER.info( "Event sourcing UnAssignTapChangerControlFromTapChangerEvent" );
    ...
    private TapSchedule tapChanger = null;
