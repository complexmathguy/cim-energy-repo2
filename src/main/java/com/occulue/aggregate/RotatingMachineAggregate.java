...
public class RotatingMachineAggregate {  
    ...
    public void handle(UpdateRotatingMachineCommand command) throws Exception {
    	...
    	UpdateRotatingMachineEvent event = new UpdateRotatingMachineEvent(command.getRotatingMachineId(), command.getRatedPowerFactor(), command.getRatedS(), command.getRatedU(), command.getGeneratingUnit());        
    ...
    @CommandHandler
    public void handle(AssignGeneratingUnitToRotatingMachineCommand command) throws Exception {
    	LOGGER.info( "Handling command AssignGeneratingUnitToRotatingMachineCommand" );
        ...
        apply(new AssignGeneratingUnitToRotatingMachineEvent(command.getRotatingMachineId(), command.getAssignment()));
    }

    @CommandHandler
    public void handle(UnAssignGeneratingUnitFromRotatingMachineCommand command) throws Exception {
    	LOGGER.info( "Handlign command UnAssignGeneratingUnitFromRotatingMachineCommand" );

    	if (  generatingUnit == null )
    		throw new ProcessingException( "GeneratingUnit already has nothing assigned." );  

    	apply(new UnAssignGeneratingUnitFromRotatingMachineEvent(command.getRotatingMachineId()));
    }
	...
	// single associations
    @EventSourcingHandler
    void on(AssignGeneratingUnitToRotatingMachineEvent event ) {	
    	LOGGER.info( "Event sourcing AssignGeneratingUnitToRotatingMachineEvent" );
    	this.generatingUnit = event.getAssignment();
    }

	@EventSourcingHandler
	void on(UnAssignGeneratingUnitFromRotatingMachineEvent event ) {	
		LOGGER.info( "Event sourcing UnAssignGeneratingUnitFromRotatingMachineEvent" );
    ...
    private HydroPump rotatingMachine = null;
