...
public class PowerTransformerEndAggregate {  
    ...
    public void handle(UpdatePowerTransformerEndCommand command) throws Exception {
    	...
    	UpdatePowerTransformerEndEvent event = new UpdatePowerTransformerEndEvent(command.getPowerTransformerEndId(), command.getB(), command.getB0(), command.getConnectionKind(), command.getG(), command.getG0(), command.getPhaseAngleClock(), command.getR(), command.getR0(), command.getRatedS(), command.getRatedU(), command.getX(), command.getX0(), command.getPowerTransformer());        
    ...
    @CommandHandler
    public void handle(AssignPowerTransformerToPowerTransformerEndCommand command) throws Exception {
    	LOGGER.info( "Handling command AssignPowerTransformerToPowerTransformerEndCommand" );
    	
    	if (  powerTransformer != null && powerTransformer.getPowerTransformerId() == command.getAssignment().getPowerTransformerId() )
    		throw new ProcessingException( "PowerTransformer already assigned with id " + command.getAssignment().getPowerTransformerId() );  
    		
        apply(new AssignPowerTransformerToPowerTransformerEndEvent(command.getPowerTransformerEndId(), command.getAssignment()));
    }

    @CommandHandler
    public void handle(UnAssignPowerTransformerFromPowerTransformerEndCommand command) throws Exception {
    	LOGGER.info( "Handlign command UnAssignPowerTransformerFromPowerTransformerEndCommand" );

    	if (  powerTransformer == null )
    		throw new ProcessingException( "PowerTransformer already has nothing assigned." );  

    	apply(new UnAssignPowerTransformerFromPowerTransformerEndEvent(command.getPowerTransformerEndId()));
    }
    ...
    void on(UpdatePowerTransformerEndEvent event) {
        ...
        this.x = event.getX();
        this.x0 = event.getX0();
        this.powerTransformer = event.getPowerTransformer();
    }   
	...
	// single associations
    @EventSourcingHandler
    void on(AssignPowerTransformerToPowerTransformerEndEvent event ) {	
    	LOGGER.info( "Event sourcing AssignPowerTransformerToPowerTransformerEndEvent" );
    	this.powerTransformer = event.getAssignment();
    }

	@EventSourcingHandler
	void on(UnAssignPowerTransformerFromPowerTransformerEndEvent event ) {	
		LOGGER.info( "Event sourcing UnAssignPowerTransformerFromPowerTransformerEndEvent" );
