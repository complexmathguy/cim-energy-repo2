...
public class LoadStaticEntityProjector {
    ...
    /*
     * Assign a LoadAggregate
     * 
     * @param	parentId	UUID
     * @param	assignment 	LoadAggregate 
     * @return	LoadStatic
     */
    public LoadStatic assignLoadAggregate( UUID parentId, LoadAggregate assignment ) {
	    LOGGER.info("assigning LoadAggregate as " + assignment.toString() );

	    LoadStatic parentEntity = repository.findById( parentId ).get();
	    assignment = LoadAggregateProjector.find(assignment.getLoadAggregateId());
	    
	    // ------------------------------------------
		// assign the LoadAggregate to the parent entity
		// ------------------------------------------ 
	    parentEntity.setLoadAggregate( assignment );
	...
	/*
	 * Unassign the LoadAggregate
	 * 
	 * @param	parentId		UUID
	 * @return	LoadStatic
	 */
	public LoadStatic unAssignLoadAggregate( UUID parentId ) {
		LoadStatic parentEntity = repository.findById(parentId).get();

		LOGGER.info("unAssigning LoadAggregate on " + parentEntity.toString() );
		
	    // ------------------------------------------
		// null out the LoadAggregate on the parent entithy
		// ------------------------------------------     
	    parentEntity.setLoadAggregate(null);
    ...
    @Autowired
	@Qualifier(value = "loadAggregate-entity-projector")
	LoadAggregateEntityProjector LoadAggregateProjector;
