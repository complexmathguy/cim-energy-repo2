...
public class FossilFuelEntityProjector {
    ...
    /*
     * Assign a ThermalGeneratingUnit
     * 
     * @param	parentId	UUID
     * @param	assignment 	ThermalGeneratingUnit 
     * @return	FossilFuel
     */
    public FossilFuel assignThermalGeneratingUnit( UUID parentId, ThermalGeneratingUnit assignment ) {
	    LOGGER.info("assigning ThermalGeneratingUnit as " + assignment.toString() );

	    FossilFuel parentEntity = repository.findById( parentId ).get();
	    assignment = ThermalGeneratingUnitProjector.find(assignment.getThermalGeneratingUnitId());
	    
	    // ------------------------------------------
		// assign the ThermalGeneratingUnit to the parent entity
		// ------------------------------------------ 
	    parentEntity.setThermalGeneratingUnit( assignment );
	...
	/*
	 * Unassign the ThermalGeneratingUnit
	 * 
	 * @param	parentId		UUID
	 * @return	FossilFuel
	 */
	public FossilFuel unAssignThermalGeneratingUnit( UUID parentId ) {
		FossilFuel parentEntity = repository.findById(parentId).get();

		LOGGER.info("unAssigning ThermalGeneratingUnit on " + parentEntity.toString() );
		
	    // ------------------------------------------
		// null out the ThermalGeneratingUnit on the parent entithy
		// ------------------------------------------     
	    parentEntity.setThermalGeneratingUnit(null);
    ...
    @Autowired
	@Qualifier(value = "thermalGeneratingUnit-entity-projector")
	ThermalGeneratingUnitEntityProjector ThermalGeneratingUnitProjector;
