...
public class EnergyConsumerEntityProjector {
    ...
    /*
     * Assign a LoadResponse
     * 
     * @param	parentId	UUID
     * @param	assignment 	LoadResponseCharacteristic 
     * @return	EnergyConsumer
     */
    public EnergyConsumer assignLoadResponse( UUID parentId, LoadResponseCharacteristic assignment ) {
	    LOGGER.info("assigning LoadResponse as " + assignment.toString() );

	    EnergyConsumer parentEntity = repository.findById( parentId ).get();
	    assignment = LoadResponseCharacteristicProjector.find(assignment.getLoadResponseCharacteristicId());
	    
	    // ------------------------------------------
		// assign the LoadResponse to the parent entity
		// ------------------------------------------ 
	    parentEntity.setLoadResponse( assignment );
	...
	/*
	 * Unassign the LoadResponse
	 * 
	 * @param	parentId		UUID
	 * @return	EnergyConsumer
	 */
	public EnergyConsumer unAssignLoadResponse( UUID parentId ) {
		EnergyConsumer parentEntity = repository.findById(parentId).get();

		LOGGER.info("unAssigning LoadResponse on " + parentEntity.toString() );
		
	    // ------------------------------------------
		// null out the LoadResponse on the parent entithy
		// ------------------------------------------     
	    parentEntity.setLoadResponse(null);
