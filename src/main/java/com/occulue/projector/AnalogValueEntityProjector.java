...
public class AnalogValueEntityProjector {
     ...
     * 
     * @param	parentId	UUID
     * @param	assignment 	Analog 
     * @return	AnalogValue
     */
    public AnalogValue assignAnalog( UUID parentId, Analog assignment ) {
	    LOGGER.info("assigning Analog as " + assignment.toString() );

	    AnalogValue parentEntity = repository.findById( parentId ).get();
	    assignment = AnalogProjector.find(assignment.getAnalogId());
	    
	    // ------------------------------------------
		// assign the Analog to the parent entity
		// ------------------------------------------ 
	    parentEntity.setAnalog( assignment );
	 ...
	 * @return	AnalogValue
	 */
	public AnalogValue unAssignAnalog( UUID parentId ) {
		AnalogValue parentEntity = repository.findById(parentId).get();

		LOGGER.info("unAssigning Analog on " + parentEntity.toString() );
		
	    // ------------------------------------------
		// null out the Analog on the parent entithy
		// ------------------------------------------     
	    parentEntity.setAnalog(null);
