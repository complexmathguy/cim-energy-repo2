...
public class WindGenTurbineType1IECEntityProjector {
    ...
    /*
     * Assign a WindAeroConstIEC
     * 
     * @param	parentId	UUID
     * @param	assignment 	WindAeroConstIEC 
     * @return	WindGenTurbineType1IEC
     */
    public WindGenTurbineType1IEC assignWindAeroConstIEC( UUID parentId, WindAeroConstIEC assignment ) {
	    LOGGER.info("assigning WindAeroConstIEC as " + assignment.toString() );

	    WindGenTurbineType1IEC parentEntity = repository.findById( parentId ).get();
	    assignment = WindAeroConstIECProjector.find(assignment.getWindAeroConstIECId());
	    
	    // ------------------------------------------
		// assign the WindAeroConstIEC to the parent entity
		// ------------------------------------------ 
	    parentEntity.setWindAeroConstIEC( assignment );
	...
	/*
	 * Unassign the WindAeroConstIEC
	 * 
	 * @param	parentId		UUID
	 * @return	WindGenTurbineType1IEC
	 */
	public WindGenTurbineType1IEC unAssignWindAeroConstIEC( UUID parentId ) {
		WindGenTurbineType1IEC parentEntity = repository.findById(parentId).get();

		LOGGER.info("unAssigning WindAeroConstIEC on " + parentEntity.toString() );
		
	    // ------------------------------------------
		// null out the WindAeroConstIEC on the parent entithy
		// ------------------------------------------     
	    parentEntity.setWindAeroConstIEC(null);
    ...
    @Autowired
	@Qualifier(value = "windAeroConstIEC-entity-projector")
	WindAeroConstIECEntityProjector WindAeroConstIECProjector;
