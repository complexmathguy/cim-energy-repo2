...
public class MeasurementValueQualityEntityProjector {
    ...
    /*
     * Assign a MeasurementValue
     * 
     * @param	parentId	UUID
     * @param	assignment 	MeasurementValue 
     * @return	MeasurementValueQuality
     */
    public MeasurementValueQuality assignMeasurementValue( UUID parentId, MeasurementValue assignment ) {
	    LOGGER.info("assigning MeasurementValue as " + assignment.toString() );

	    MeasurementValueQuality parentEntity = repository.findById( parentId ).get();
	    assignment = MeasurementValueProjector.find(assignment.getMeasurementValueId());
	    
	    // ------------------------------------------
		// assign the MeasurementValue to the parent entity
		// ------------------------------------------ 
	    parentEntity.setMeasurementValue( assignment );
	...
	/*
	 * Unassign the MeasurementValue
	 * 
	 * @param	parentId		UUID
	 * @return	MeasurementValueQuality
	 */
	public MeasurementValueQuality unAssignMeasurementValue( UUID parentId ) {
		MeasurementValueQuality parentEntity = repository.findById(parentId).get();

		LOGGER.info("unAssigning MeasurementValue on " + parentEntity.toString() );
		
	    // ------------------------------------------
		// null out the MeasurementValue on the parent entithy
		// ------------------------------------------     
	    parentEntity.setMeasurementValue(null);
