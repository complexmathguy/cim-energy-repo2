...
public class DiscreteValueEntityProjector {
     ...
     * 
     * @param	parentId	UUID
     * @param	assignment 	Discrete 
     * @return	DiscreteValue
     */
    public DiscreteValue assignDiscrete( UUID parentId, Discrete assignment ) {
	    LOGGER.info("assigning Discrete as " + assignment.toString() );

	    DiscreteValue parentEntity = repository.findById( parentId ).get();
	    assignment = DiscreteProjector.find(assignment.getDiscreteId());
	    
	    // ------------------------------------------
		// assign the Discrete to the parent entity
		// ------------------------------------------ 
	    parentEntity.setDiscrete( assignment );
	...
	/*
	 * Unassign the Discrete
	 * 
	 * @param	parentId		UUID
	 * @return	DiscreteValue
	 */
	public DiscreteValue unAssignDiscrete( UUID parentId ) {
		DiscreteValue parentEntity = repository.findById(parentId).get();

		LOGGER.info("unAssigning Discrete on " + parentEntity.toString() );
		
	    // ------------------------------------------
		// null out the Discrete on the parent entithy
		// ------------------------------------------     
	    parentEntity.setDiscrete(null);
