...
public class AsynchronousMachineDynamicsEntityProjector {
    ...
    /*
     * Assign a AsynchronousMachine
     * 
     * @param	parentId	UUID
     * @param	assignment 	AsynchronousMachine 
     * @return	AsynchronousMachineDynamics
     */
    public AsynchronousMachineDynamics assignAsynchronousMachine( UUID parentId, AsynchronousMachine assignment ) {
	    LOGGER.info("assigning AsynchronousMachine as " + assignment.toString() );

	    AsynchronousMachineDynamics parentEntity = repository.findById( parentId ).get();
	    assignment = AsynchronousMachineProjector.find(assignment.getAsynchronousMachineId());
	    
	    // ------------------------------------------
		// assign the AsynchronousMachine to the parent entity
		// ------------------------------------------ 
	    parentEntity.setAsynchronousMachine( assignment );
	...
	/*
	 * Unassign the AsynchronousMachine
	 * 
	 * @param	parentId		UUID
	 * @return	AsynchronousMachineDynamics
	 */
	public AsynchronousMachineDynamics unAssignAsynchronousMachine( UUID parentId ) {
		AsynchronousMachineDynamics parentEntity = repository.findById(parentId).get();

		LOGGER.info("unAssigning AsynchronousMachine on " + parentEntity.toString() );
		
	    // ------------------------------------------
		// null out the AsynchronousMachine on the parent entithy
		// ------------------------------------------     
	    parentEntity.setAsynchronousMachine(null);
