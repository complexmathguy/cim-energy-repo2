...
public class AccumulatorResetEntityProjector {
    ...
    /*
     * Assign a AccumulatorValue
     * 
     * @param	parentId	UUID
     * @param	assignment 	AccumulatorValue 
     * @return	AccumulatorReset
     */
    public AccumulatorReset assignAccumulatorValue( UUID parentId, AccumulatorValue assignment ) {
	    LOGGER.info("assigning AccumulatorValue as " + assignment.toString() );

	    AccumulatorReset parentEntity = repository.findById( parentId ).get();
	    assignment = AccumulatorValueProjector.find(assignment.getAccumulatorValueId());
	    
	    // ------------------------------------------
		// assign the AccumulatorValue to the parent entity
		// ------------------------------------------ 
	    parentEntity.setAccumulatorValue( assignment );
	...
	/*
	 * Unassign the AccumulatorValue
	 * 
	 * @param	parentId		UUID
	 * @return	AccumulatorReset
	 */
	public AccumulatorReset unAssignAccumulatorValue( UUID parentId ) {
		AccumulatorReset parentEntity = repository.findById(parentId).get();

		LOGGER.info("unAssigning AccumulatorValue on " + parentEntity.toString() );
		
	    // ------------------------------------------
		// null out the AccumulatorValue on the parent entithy
		// ------------------------------------------     
	    parentEntity.setAccumulatorValue(null);
