...
public class ControlAreaEntityProjector {
     ...
     * 
     * @param	parentId	UUID
     * @param	assignment 	EnergyArea 
     * @return	ControlArea
     */
    public ControlArea assignEnergyArea( UUID parentId, EnergyArea assignment ) {
	    LOGGER.info("assigning EnergyArea as " + assignment.toString() );

	    ControlArea parentEntity = repository.findById( parentId ).get();
	    assignment = EnergyAreaProjector.find(assignment.getEnergyAreaId());
	    
	    // ------------------------------------------
		// assign the EnergyArea to the parent entity
		// ------------------------------------------ 
	    parentEntity.setEnergyArea( assignment );
	...
	/*
	 * Unassign the EnergyArea
	 * 
	 * @param	parentId		UUID
	 * @return	ControlArea
	 */
	public ControlArea unAssignEnergyArea( UUID parentId ) {
		ControlArea parentEntity = repository.findById(parentId).get();

		LOGGER.info("unAssigning EnergyArea on " + parentEntity.toString() );
		
	    // ------------------------------------------
		// null out the EnergyArea on the parent entithy
		// ------------------------------------------     
	    parentEntity.setEnergyArea(null);
    ...
    @Autowired
	@Qualifier(value = "energyArea-entity-projector")
	EnergyAreaEntityProjector EnergyAreaProjector;
    @Autowired
