...
public class DiagramEntityProjector {
    ...
    /*
     * Assign a DiagramStyle
     * 
     * @param	parentId	UUID
     * @param	assignment 	DiagramStyle 
     * @return	Diagram
     */
    public Diagram assignDiagramStyle( UUID parentId, DiagramStyle assignment ) {
	    LOGGER.info("assigning DiagramStyle as " + assignment.toString() );

	    Diagram parentEntity = repository.findById( parentId ).get();
	    assignment = DiagramStyleProjector.find(assignment.getDiagramStyleId());
	    
	    // ------------------------------------------
		// assign the DiagramStyle to the parent entity
		// ------------------------------------------ 
	    parentEntity.setDiagramStyle( assignment );
	...
	/*
	 * Unassign the DiagramStyle
	 * 
	 * @param	parentId		UUID
	 * @return	Diagram
	 */
	public Diagram unAssignDiagramStyle( UUID parentId ) {
		Diagram parentEntity = repository.findById(parentId).get();

		LOGGER.info("unAssigning DiagramStyle on " + parentEntity.toString() );
		
	    // ------------------------------------------
		// null out the DiagramStyle on the parent entithy
		// ------------------------------------------     
	    parentEntity.setDiagramStyle(null);
    ...
    @Autowired
	@Qualifier(value = "diagramStyle-entity-projector")
	DiagramStyleEntityProjector DiagramStyleProjector;
    @Autowired
