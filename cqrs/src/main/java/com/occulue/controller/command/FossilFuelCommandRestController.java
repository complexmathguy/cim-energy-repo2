...
public class FossilFuelCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save ThermalGeneratingUnit on FossilFuel
     * @param		command AssignThermalGeneratingUnitToFossilFuelCommand
     */     
	@PutMapping("/assignThermalGeneratingUnit")
	public void assignThermalGeneratingUnit( @RequestBody AssignThermalGeneratingUnitToFossilFuelCommand command ) {
		try {
			FossilFuelBusinessDelegate.getFossilFuelInstance().assignThermalGeneratingUnit( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign ThermalGeneratingUnit", exc );
        }
	}

    /**
     * unassign ThermalGeneratingUnit on FossilFuel
     * @param		 command UnAssignThermalGeneratingUnitFromFossilFuelCommand
     */     
	@PutMapping("/unAssignThermalGeneratingUnit")
	public void unAssignThermalGeneratingUnit( @RequestBody(required=true)  UnAssignThermalGeneratingUnitFromFossilFuelCommand command ) {
		try {
			FossilFuelBusinessDelegate.getFossilFuelInstance().unAssignThermalGeneratingUnit( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign ThermalGeneratingUnit", exc );
		}
	}
