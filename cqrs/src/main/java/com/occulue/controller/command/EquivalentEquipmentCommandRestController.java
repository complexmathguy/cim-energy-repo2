...
public class EquivalentEquipmentCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save EquivalentNetwork on EquivalentEquipment
     * @param		command AssignEquivalentNetworkToEquivalentEquipmentCommand
     */     
	@PutMapping("/assignEquivalentNetwork")
	public void assignEquivalentNetwork( @RequestBody AssignEquivalentNetworkToEquivalentEquipmentCommand command ) {
		try {
			EquivalentEquipmentBusinessDelegate.getEquivalentEquipmentInstance().assignEquivalentNetwork( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign EquivalentNetwork", exc );
        }
	}

    /**
     * unassign EquivalentNetwork on EquivalentEquipment
     * @param		 command UnAssignEquivalentNetworkFromEquivalentEquipmentCommand
     */     
	@PutMapping("/unAssignEquivalentNetwork")
	public void unAssignEquivalentNetwork( @RequestBody(required=true)  UnAssignEquivalentNetworkFromEquivalentEquipmentCommand command ) {
		try {
			EquivalentEquipmentBusinessDelegate.getEquivalentEquipmentInstance().unAssignEquivalentNetwork( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign EquivalentNetwork", exc );
		}
	}
