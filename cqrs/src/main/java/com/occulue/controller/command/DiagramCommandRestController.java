...
public class DiagramCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save DiagramStyle on Diagram
     * @param		command AssignDiagramStyleToDiagramCommand
     */     
	@PutMapping("/assignDiagramStyle")
	public void assignDiagramStyle( @RequestBody AssignDiagramStyleToDiagramCommand command ) {
		try {
			DiagramBusinessDelegate.getDiagramInstance().assignDiagramStyle( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign DiagramStyle", exc );
        }
	}

    /**
     * unassign DiagramStyle on Diagram
     * @param		 command UnAssignDiagramStyleFromDiagramCommand
     */     
	@PutMapping("/unAssignDiagramStyle")
	public void unAssignDiagramStyle( @RequestBody(required=true)  UnAssignDiagramStyleFromDiagramCommand command ) {
		try {
			DiagramBusinessDelegate.getDiagramInstance().unAssignDiagramStyle( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign DiagramStyle", exc );
		}
	}
