...
public class DCBaseTerminalCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save DCNode on DCBaseTerminal
     * @param		command AssignDCNodeToDCBaseTerminalCommand
     */     
	@PutMapping("/assignDCNode")
	public void assignDCNode( @RequestBody AssignDCNodeToDCBaseTerminalCommand command ) {
		try {
			DCBaseTerminalBusinessDelegate.getDCBaseTerminalInstance().assignDCNode( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign DCNode", exc );
        }
	}

    /**
     * unassign DCNode on DCBaseTerminal
     * @param		 command UnAssignDCNodeFromDCBaseTerminalCommand
     */     
	@PutMapping("/unAssignDCNode")
	public void unAssignDCNode( @RequestBody(required=true)  UnAssignDCNodeFromDCBaseTerminalCommand command ) {
		try {
			DCBaseTerminalBusinessDelegate.getDCBaseTerminalInstance().unAssignDCNode( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign DCNode", exc );
		}
	}
