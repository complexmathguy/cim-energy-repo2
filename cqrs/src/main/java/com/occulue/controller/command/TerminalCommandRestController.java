...
public class TerminalCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save ConductingEquipment on Terminal
     * @param		command AssignConductingEquipmentToTerminalCommand
     */     
	@PutMapping("/assignConductingEquipment")
	public void assignConductingEquipment( @RequestBody AssignConductingEquipmentToTerminalCommand command ) {
		try {
			TerminalBusinessDelegate.getTerminalInstance().assignConductingEquipment( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign ConductingEquipment", exc );
        }
	}

    /**
     * unassign ConductingEquipment on Terminal
     * @param		 command UnAssignConductingEquipmentFromTerminalCommand
     */     
	@PutMapping("/unAssignConductingEquipment")
	public void unAssignConductingEquipment( @RequestBody(required=true)  UnAssignConductingEquipmentFromTerminalCommand command ) {
		try {
			TerminalBusinessDelegate.getTerminalInstance().unAssignConductingEquipment( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign ConductingEquipment", exc );
		}
	}
