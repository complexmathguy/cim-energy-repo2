...
public class SubLoadAreaCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save LoadArea on SubLoadArea
     * @param		command AssignLoadAreaToSubLoadAreaCommand
     */     
	@PutMapping("/assignLoadArea")
	public void assignLoadArea( @RequestBody AssignLoadAreaToSubLoadAreaCommand command ) {
		try {
			SubLoadAreaBusinessDelegate.getSubLoadAreaInstance().assignLoadArea( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign LoadArea", exc );
        }
	}

    /**
     * unassign LoadArea on SubLoadArea
     * @param		 command UnAssignLoadAreaFromSubLoadAreaCommand
     */     
	@PutMapping("/unAssignLoadArea")
	public void unAssignLoadArea( @RequestBody(required=true)  UnAssignLoadAreaFromSubLoadAreaCommand command ) {
		try {
			SubLoadAreaBusinessDelegate.getSubLoadAreaInstance().unAssignLoadArea( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign LoadArea", exc );
		}
	}
