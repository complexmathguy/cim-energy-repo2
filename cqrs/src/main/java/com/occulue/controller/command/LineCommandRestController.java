...
public class LineCommandRestController extends BaseSpringRestController {
	...
	public void assignRegion( @RequestBody AssignRegionToLineCommand command ) {
		try {
			LineBusinessDelegate.getLineInstance().assignRegion( command );   
		}
        catch( Throwable exc ) {
	...
	public void unAssignRegion( @RequestBody(required=true)  UnAssignRegionFromLineCommand command ) {
		try {
			LineBusinessDelegate.getLineInstance().unAssignRegion( command );   
		}
		catch( Exception exc ) {
