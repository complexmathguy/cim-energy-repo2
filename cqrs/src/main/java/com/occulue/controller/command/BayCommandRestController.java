...
public class BayCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save VoltageLevel on Bay
     * @param		command AssignVoltageLevelToBayCommand
     */     
	@PutMapping("/assignVoltageLevel")
	public void assignVoltageLevel( @RequestBody AssignVoltageLevelToBayCommand command ) {
		try {
			BayBusinessDelegate.getBayInstance().assignVoltageLevel( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign VoltageLevel", exc );
        }
	}

    /**
     * unassign VoltageLevel on Bay
     * @param		 command UnAssignVoltageLevelFromBayCommand
     */     
	@PutMapping("/unAssignVoltageLevel")
	public void unAssignVoltageLevel( @RequestBody(required=true)  UnAssignVoltageLevelFromBayCommand command ) {
		try {
			BayBusinessDelegate.getBayInstance().unAssignVoltageLevel( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign VoltageLevel", exc );
		}
	}
