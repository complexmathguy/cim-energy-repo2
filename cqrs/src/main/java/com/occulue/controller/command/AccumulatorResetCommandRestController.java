...
public class AccumulatorResetCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save AccumulatorValue on AccumulatorReset
     * @param		command AssignAccumulatorValueToAccumulatorResetCommand
     */     
	@PutMapping("/assignAccumulatorValue")
	public void assignAccumulatorValue( @RequestBody AssignAccumulatorValueToAccumulatorResetCommand command ) {
		try {
			AccumulatorResetBusinessDelegate.getAccumulatorResetInstance().assignAccumulatorValue( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign AccumulatorValue", exc );
        }
	}

    /**
     * unassign AccumulatorValue on AccumulatorReset
     * @param		 command UnAssignAccumulatorValueFromAccumulatorResetCommand
     */     
	@PutMapping("/unAssignAccumulatorValue")
	public void unAssignAccumulatorValue( @RequestBody(required=true)  UnAssignAccumulatorValueFromAccumulatorResetCommand command ) {
		try {
			AccumulatorResetBusinessDelegate.getAccumulatorResetInstance().unAssignAccumulatorValue( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign AccumulatorValue", exc );
		}
	}
