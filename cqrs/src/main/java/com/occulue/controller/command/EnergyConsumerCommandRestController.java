...
public class EnergyConsumerCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save LoadResponse on EnergyConsumer
     * @param		command AssignLoadResponseToEnergyConsumerCommand
     */     
	@PutMapping("/assignLoadResponse")
	public void assignLoadResponse( @RequestBody AssignLoadResponseToEnergyConsumerCommand command ) {
		try {
			EnergyConsumerBusinessDelegate.getEnergyConsumerInstance().assignLoadResponse( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign LoadResponse", exc );
        }
	}
     ...
     */     
	@PutMapping("/unAssignLoadResponse")
	public void unAssignLoadResponse( @RequestBody(required=true)  UnAssignLoadResponseFromEnergyConsumerCommand command ) {
		try {
			EnergyConsumerBusinessDelegate.getEnergyConsumerInstance().unAssignLoadResponse( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign LoadResponse", exc );
		}
	}
