...
public class WindGenTurbineType1IECCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save WindAeroConstIEC on WindGenTurbineType1IEC
     * @param		command AssignWindAeroConstIECToWindGenTurbineType1IECCommand
     */     
	@PutMapping("/assignWindAeroConstIEC")
	public void assignWindAeroConstIEC( @RequestBody AssignWindAeroConstIECToWindGenTurbineType1IECCommand command ) {
		try {
			WindGenTurbineType1IECBusinessDelegate.getWindGenTurbineType1IECInstance().assignWindAeroConstIEC( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign WindAeroConstIEC", exc );
        }
	}

    /**
     * unassign WindAeroConstIEC on WindGenTurbineType1IEC
     * @param		 command UnAssignWindAeroConstIECFromWindGenTurbineType1IECCommand
     */     
	@PutMapping("/unAssignWindAeroConstIEC")
	public void unAssignWindAeroConstIEC( @RequestBody(required=true)  UnAssignWindAeroConstIECFromWindGenTurbineType1IECCommand command ) {
		try {
			WindGenTurbineType1IECBusinessDelegate.getWindGenTurbineType1IECInstance().unAssignWindAeroConstIEC( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign WindAeroConstIEC", exc );
		}
	}
