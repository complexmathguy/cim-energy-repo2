...
public class DCTopologicalNodeCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save DCEquipmentContainer on DCTopologicalNode
     * @param		command AssignDCEquipmentContainerToDCTopologicalNodeCommand
     */     
	@PutMapping("/assignDCEquipmentContainer")
	public void assignDCEquipmentContainer( @RequestBody AssignDCEquipmentContainerToDCTopologicalNodeCommand command ) {
		try {
			DCTopologicalNodeBusinessDelegate.getDCTopologicalNodeInstance().assignDCEquipmentContainer( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign DCEquipmentContainer", exc );
        }
	}

    /**
     * unassign DCEquipmentContainer on DCTopologicalNode
     * @param		 command UnAssignDCEquipmentContainerFromDCTopologicalNodeCommand
     */     
	@PutMapping("/unAssignDCEquipmentContainer")
	public void unAssignDCEquipmentContainer( @RequestBody(required=true)  UnAssignDCEquipmentContainerFromDCTopologicalNodeCommand command ) {
		try {
			DCTopologicalNodeBusinessDelegate.getDCTopologicalNodeInstance().unAssignDCEquipmentContainer( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign DCEquipmentContainer", exc );
		}
	}
