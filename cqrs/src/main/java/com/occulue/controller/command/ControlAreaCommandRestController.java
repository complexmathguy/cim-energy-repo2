...
public class ControlAreaCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save EnergyArea on ControlArea
     * @param		command AssignEnergyAreaToControlAreaCommand
     */     
	@PutMapping("/assignEnergyArea")
	public void assignEnergyArea( @RequestBody AssignEnergyAreaToControlAreaCommand command ) {
		try {
			ControlAreaBusinessDelegate.getControlAreaInstance().assignEnergyArea( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign EnergyArea", exc );
        }
	}

    /**
     * unassign EnergyArea on ControlArea
     * @param		 command UnAssignEnergyAreaFromControlAreaCommand
     */     
	@PutMapping("/unAssignEnergyArea")
	public void unAssignEnergyArea( @RequestBody(required=true)  UnAssignEnergyAreaFromControlAreaCommand command ) {
		try {
			ControlAreaBusinessDelegate.getControlAreaInstance().unAssignEnergyArea( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign EnergyArea", exc );
		}
	}
