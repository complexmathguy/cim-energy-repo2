...
public class EquipmentCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save EquipmentContainer on Equipment
     * @param		command AssignEquipmentContainerToEquipmentCommand
     */     
	@PutMapping("/assignEquipmentContainer")
	public void assignEquipmentContainer( @RequestBody AssignEquipmentContainerToEquipmentCommand command ) {
		try {
			EquipmentBusinessDelegate.getEquipmentInstance().assignEquipmentContainer( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign EquipmentContainer", exc );
        }
	}

    /**
     * unassign EquipmentContainer on Equipment
     * @param		 command UnAssignEquipmentContainerFromEquipmentCommand
     */     
	@PutMapping("/unAssignEquipmentContainer")
	public void unAssignEquipmentContainer( @RequestBody(required=true)  UnAssignEquipmentContainerFromEquipmentCommand command ) {
		try {
			EquipmentBusinessDelegate.getEquipmentInstance().unAssignEquipmentContainer( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign EquipmentContainer", exc );
		}
	}
