...
public class AsynchronousMachineDynamicsCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save AsynchronousMachine on AsynchronousMachineDynamics
     * @param		command AssignAsynchronousMachineToAsynchronousMachineDynamicsCommand
     */     
	@PutMapping("/assignAsynchronousMachine")
	public void assignAsynchronousMachine( @RequestBody AssignAsynchronousMachineToAsynchronousMachineDynamicsCommand command ) {
		try {
			AsynchronousMachineDynamicsBusinessDelegate.getAsynchronousMachineDynamicsInstance().assignAsynchronousMachine( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign AsynchronousMachine", exc );
        }
	}

    /**
     * unassign AsynchronousMachine on AsynchronousMachineDynamics
     * @param		 command UnAssignAsynchronousMachineFromAsynchronousMachineDynamicsCommand
     */     
	@PutMapping("/unAssignAsynchronousMachine")
	public void unAssignAsynchronousMachine( @RequestBody(required=true)  UnAssignAsynchronousMachineFromAsynchronousMachineDynamicsCommand command ) {
		try {
			AsynchronousMachineDynamicsBusinessDelegate.getAsynchronousMachineDynamicsInstance().unAssignAsynchronousMachine( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign AsynchronousMachine", exc );
		}
	}
