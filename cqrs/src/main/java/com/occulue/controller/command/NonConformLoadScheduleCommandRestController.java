...
public class NonConformLoadScheduleCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save NonConformLoadGroup on NonConformLoadSchedule
     * @param		command AssignNonConformLoadGroupToNonConformLoadScheduleCommand
     */     
	@PutMapping("/assignNonConformLoadGroup")
	public void assignNonConformLoadGroup( @RequestBody AssignNonConformLoadGroupToNonConformLoadScheduleCommand command ) {
		try {
			NonConformLoadScheduleBusinessDelegate.getNonConformLoadScheduleInstance().assignNonConformLoadGroup( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign NonConformLoadGroup", exc );
        }
	}

    /**
     * unassign NonConformLoadGroup on NonConformLoadSchedule
     * @param		 command UnAssignNonConformLoadGroupFromNonConformLoadScheduleCommand
     */     
	@PutMapping("/unAssignNonConformLoadGroup")
	public void unAssignNonConformLoadGroup( @RequestBody(required=true)  UnAssignNonConformLoadGroupFromNonConformLoadScheduleCommand command ) {
		try {
			NonConformLoadScheduleBusinessDelegate.getNonConformLoadScheduleInstance().unAssignNonConformLoadGroup( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign NonConformLoadGroup", exc );
		}
	}
