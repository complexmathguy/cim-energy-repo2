...
public class MeasurementValueQualityCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save MeasurementValue on MeasurementValueQuality
     * @param		command AssignMeasurementValueToMeasurementValueQualityCommand
     */     
	@PutMapping("/assignMeasurementValue")
	public void assignMeasurementValue( @RequestBody AssignMeasurementValueToMeasurementValueQualityCommand command ) {
		try {
			MeasurementValueQualityBusinessDelegate.getMeasurementValueQualityInstance().assignMeasurementValue( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign MeasurementValue", exc );
        }
	}

    /**
     * unassign MeasurementValue on MeasurementValueQuality
     * @param		 command UnAssignMeasurementValueFromMeasurementValueQualityCommand
     */     
	@PutMapping("/unAssignMeasurementValue")
	public void unAssignMeasurementValue( @RequestBody(required=true)  UnAssignMeasurementValueFromMeasurementValueQualityCommand command ) {
		try {
			MeasurementValueQualityBusinessDelegate.getMeasurementValueQualityInstance().unAssignMeasurementValue( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign MeasurementValue", exc );
		}
	}
