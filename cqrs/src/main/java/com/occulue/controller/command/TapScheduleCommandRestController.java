...
public class TapScheduleCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save TapChanger on TapSchedule
     * @param		command AssignTapChangerToTapScheduleCommand
     */     
	@PutMapping("/assignTapChanger")
	public void assignTapChanger( @RequestBody AssignTapChangerToTapScheduleCommand command ) {
		try {
			TapScheduleBusinessDelegate.getTapScheduleInstance().assignTapChanger( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign TapChanger", exc );
        }
	}

    /**
     * unassign TapChanger on TapSchedule
     * @param		 command UnAssignTapChangerFromTapScheduleCommand
     */     
	@PutMapping("/unAssignTapChanger")
	public void unAssignTapChanger( @RequestBody(required=true)  UnAssignTapChangerFromTapScheduleCommand command ) {
		try {
			TapScheduleBusinessDelegate.getTapScheduleInstance().unAssignTapChanger( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign TapChanger", exc );
		}
	}
