...
public class SubstationCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save Region on Substation
     * @param		command AssignRegionToSubstationCommand
     */     
	@PutMapping("/assignRegion")
	public void assignRegion( @RequestBody AssignRegionToSubstationCommand command ) {
		try {
			SubstationBusinessDelegate.getSubstationInstance().assignRegion( command );   
		}
        catch( Throwable exc ) {
	...
	public void unAssignRegion( @RequestBody(required=true)  UnAssignRegionFromSubstationCommand command ) {
		try {
			SubstationBusinessDelegate.getSubstationInstance().unAssignRegion( command );   
		}
		catch( Exception exc ) {
