...
public class LoadGroupCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save SubLoadArea on LoadGroup
     * @param		command AssignSubLoadAreaToLoadGroupCommand
     */     
	@PutMapping("/assignSubLoadArea")
	public void assignSubLoadArea( @RequestBody AssignSubLoadAreaToLoadGroupCommand command ) {
		try {
			LoadGroupBusinessDelegate.getLoadGroupInstance().assignSubLoadArea( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign SubLoadArea", exc );
        }
	}

    /**
     * unassign SubLoadArea on LoadGroup
     * @param		 command UnAssignSubLoadAreaFromLoadGroupCommand
     */     
	@PutMapping("/unAssignSubLoadArea")
	public void unAssignSubLoadArea( @RequestBody(required=true)  UnAssignSubLoadAreaFromLoadGroupCommand command ) {
		try {
			LoadGroupBusinessDelegate.getLoadGroupInstance().unAssignSubLoadArea( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign SubLoadArea", exc );
		}
	}
