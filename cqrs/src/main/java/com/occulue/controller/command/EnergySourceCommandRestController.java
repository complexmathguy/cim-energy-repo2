...
public class EnergySourceCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save EnergySchedulingType on EnergySource
     * @param		command AssignEnergySchedulingTypeToEnergySourceCommand
     */     
	@PutMapping("/assignEnergySchedulingType")
	public void assignEnergySchedulingType( @RequestBody AssignEnergySchedulingTypeToEnergySourceCommand command ) {
		try {
			EnergySourceBusinessDelegate.getEnergySourceInstance().assignEnergySchedulingType( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign EnergySchedulingType", exc );
        }
	}

    /**
     * unassign EnergySchedulingType on EnergySource
     * @param		 command UnAssignEnergySchedulingTypeFromEnergySourceCommand
     */     
	@PutMapping("/unAssignEnergySchedulingType")
	public void unAssignEnergySchedulingType( @RequestBody(required=true)  UnAssignEnergySchedulingTypeFromEnergySourceCommand command ) {
		try {
			EnergySourceBusinessDelegate.getEnergySourceInstance().unAssignEnergySchedulingType( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign EnergySchedulingType", exc );
		}
	}
