...
public class ACDCConverterCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save PccTerminal on ACDCConverter
     * @param		command AssignPccTerminalToACDCConverterCommand
     */     
	@PutMapping("/assignPccTerminal")
	public void assignPccTerminal( @RequestBody AssignPccTerminalToACDCConverterCommand command ) {
		try {
			ACDCConverterBusinessDelegate.getACDCConverterInstance().assignPccTerminal( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign PccTerminal", exc );
        }
	}

    /**
     * unassign PccTerminal on ACDCConverter
     * @param		 command UnAssignPccTerminalFromACDCConverterCommand
     */     
	@PutMapping("/unAssignPccTerminal")
	public void unAssignPccTerminal( @RequestBody(required=true)  UnAssignPccTerminalFromACDCConverterCommand command ) {
		try {
			ACDCConverterBusinessDelegate.getACDCConverterInstance().unAssignPccTerminal( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign PccTerminal", exc );
		}
	}
