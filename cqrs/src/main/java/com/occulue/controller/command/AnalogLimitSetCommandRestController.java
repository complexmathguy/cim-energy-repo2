...
public class AnalogLimitSetCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save Measurements on AnalogLimitSet
     * @param		command AssignMeasurementsToAnalogLimitSetCommand
     */     
	@PutMapping("/assignMeasurements")
	public void assignMeasurements( @RequestBody AssignMeasurementsToAnalogLimitSetCommand command ) {
		try {
			AnalogLimitSetBusinessDelegate.getAnalogLimitSetInstance().assignMeasurements( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign Measurements", exc );
        }
	}

    /**
     * unassign Measurements on AnalogLimitSet
     * @param		 command UnAssignMeasurementsFromAnalogLimitSetCommand
     */     
	@PutMapping("/unAssignMeasurements")
	public void unAssignMeasurements( @RequestBody(required=true)  UnAssignMeasurementsFromAnalogLimitSetCommand command ) {
		try {
			AnalogLimitSetBusinessDelegate.getAnalogLimitSetInstance().unAssignMeasurements( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign Measurements", exc );
		}
	}
