...
public class LoadStaticCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save LoadAggregate on LoadStatic
     * @param		command AssignLoadAggregateToLoadStaticCommand
     */     
	@PutMapping("/assignLoadAggregate")
	public void assignLoadAggregate( @RequestBody AssignLoadAggregateToLoadStaticCommand command ) {
		try {
			LoadStaticBusinessDelegate.getLoadStaticInstance().assignLoadAggregate( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign LoadAggregate", exc );
        }
	}

    /**
     * unassign LoadAggregate on LoadStatic
     * @param		 command UnAssignLoadAggregateFromLoadStaticCommand
     */     
	@PutMapping("/unAssignLoadAggregate")
	public void unAssignLoadAggregate( @RequestBody(required=true)  UnAssignLoadAggregateFromLoadStaticCommand command ) {
		try {
			LoadStaticBusinessDelegate.getLoadStaticInstance().unAssignLoadAggregate( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign LoadAggregate", exc );
		}
	}
