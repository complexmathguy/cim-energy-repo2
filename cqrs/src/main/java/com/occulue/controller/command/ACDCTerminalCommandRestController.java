...
public class ACDCTerminalCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save BusNameMarker on ACDCTerminal
     * @param		command AssignBusNameMarkerToACDCTerminalCommand
     */     
	@PutMapping("/assignBusNameMarker")
	public void assignBusNameMarker( @RequestBody AssignBusNameMarkerToACDCTerminalCommand command ) {
		try {
			ACDCTerminalBusinessDelegate.getACDCTerminalInstance().assignBusNameMarker( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign BusNameMarker", exc );
        }
	}

    /**
     * unassign BusNameMarker on ACDCTerminal
     * @param		 command UnAssignBusNameMarkerFromACDCTerminalCommand
     */     
	@PutMapping("/unAssignBusNameMarker")
	public void unAssignBusNameMarker( @RequestBody(required=true)  UnAssignBusNameMarkerFromACDCTerminalCommand command ) {
		try {
			ACDCTerminalBusinessDelegate.getACDCTerminalInstance().unAssignBusNameMarker( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign BusNameMarker", exc );
		}
	}
