...
public class ConformLoadScheduleCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save ConformLoadGroup on ConformLoadSchedule
     * @param		command AssignConformLoadGroupToConformLoadScheduleCommand
     */     
	@PutMapping("/assignConformLoadGroup")
	public void assignConformLoadGroup( @RequestBody AssignConformLoadGroupToConformLoadScheduleCommand command ) {
		try {
			ConformLoadScheduleBusinessDelegate.getConformLoadScheduleInstance().assignConformLoadGroup( command );   
		}
        catch( Throwable exc ) {
        	LOGGER.log( Level.WARNING, "Failed to assign ConformLoadGroup", exc );
        }
	}

    /**
     * unassign ConformLoadGroup on ConformLoadSchedule
     * @param		 command UnAssignConformLoadGroupFromConformLoadScheduleCommand
     */     
	@PutMapping("/unAssignConformLoadGroup")
	public void unAssignConformLoadGroup( @RequestBody(required=true)  UnAssignConformLoadGroupFromConformLoadScheduleCommand command ) {
		try {
			ConformLoadScheduleBusinessDelegate.getConformLoadScheduleInstance().unAssignConformLoadGroup( command );   
		}
		catch( Exception exc ) {
			LOGGER.log( Level.WARNING, "Failed to unassign ConformLoadGroup", exc );
		}
	}
