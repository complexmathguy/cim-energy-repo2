...
public class DCLineCommandRestController extends BaseSpringRestController {
    ...
    /**
     * save Region on DCLine
     * @param		command AssignRegionToDCLineCommand
     */     
	@PutMapping("/assignRegion")
	public void assignRegion( @RequestBody AssignRegionToDCLineCommand command ) {
		try {
			DCLineBusinessDelegate.getDCLineInstance().assignRegion( command );   
		}
        catch( Throwable exc ) {
	...
	public void unAssignRegion( @RequestBody(required=true)  UnAssignRegionFromDCLineCommand command ) {
		try {
			DCLineBusinessDelegate.getDCLineInstance().unAssignRegion( command );   
		}
		catch( Exception exc ) {
